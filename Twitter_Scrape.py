...
def addTweet(conn, job_id, tweet) :
	...
	prefix = "INSERT INTO tweet (tweet_id_str, job_id, created_at, text, retweet, word_count, from_user, from_user_id_str, " \
	...
	values = [
		...
		tweet["text"],
		retweet, 								# calculated dimension
		wordcount,								# calculated dimension
		tweet["user"]["id"],
		tweet["user"]["id_str"],
	...
	if tweet['geo'] is not None:
		prefix = prefix + ", location_geo_0, location_geo_1"
		suffix = suffix + ", %s, %s"
		values.extend([
...
if __name__ == '__main__' :
	...
	parser = argparse.ArgumentParser(description="A Python Twitter Scraper")
